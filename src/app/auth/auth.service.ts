...
export class AuthService {
  ...
  getUserInfo(): Observable<User | null> {
    ...
    const userRequest = this.http
      .get<User[]>(`${apiUrl}utilisateurs`)
      .pipe(map((users) => users.find((c) => c.id === token) || null));

    if (role === 'docteur') {
      const doctorRequest = this.http
        .get<Docteur[]>(`${apiUrl}docteurs`)
        .pipe(
          map(
            (docteurs) =>
              docteurs.find((d) => d.id_utilisateur === token) || null
          )
        );
